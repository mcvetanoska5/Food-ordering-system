            ...
            .authorizeHttpRequests { auth ->
                auth
                    .requestMatchers(
                        ...
                        // Used by the MCP/Order flow to check
                        // menu-item availability before creating an order
                        "/api/menu-items/availability",

                        // Used by the demo client to fetch
                        // a real restaurant and its menu
                        "/api/restaurants/**"
                    )
                    .permitAll()
                    .anyRequest()
    ...
    private fun jwtAuthConverter(): JwtAuthenticationConverter {
        ...
        val realmRolesConverter =
            Converter<Jwt, Collection<GrantedAuthority>> { jwt ->
                ...
                val roles =
                    realmAccess?.get("roles") as? List<String>
                        ?: emptyList()
